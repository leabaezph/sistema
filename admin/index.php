...
$stmt=$mysqli->prepare("SELECT username,email,password,id FROM admin WHERE (userName=?|| email=?) and password=? ");
   ...
   // $stmtins->bind_param('sH',$id,$uip);
    //$res=$stmtins->execute();
				...
				{
					echo "<script>alert('Nombre de usuario/correo electrónico o contraseña no válidos');</script>";
				}
			}
				?>
			...
			<div class="container">
				<div class="row">
					<div class="col-md-6 col-md-offset-3" style="margin-top:4%">
						<h1 class="text-center text-bold text-light mt-4x">Sistema de Reporte a Personas Desaparecidas ADMIN</h1>
						<div class="well row pt-2x pb-3x bk-light">
							<div class="col-md-8 col-md-offset-2">
							
								<form action="" class="mt" method="post">
									<label for="" class="text-uppercase text-sm"> Email</label>
									<input type="text" placeholder="" name="username" class="form-control mb">
									<label for="" class="text-uppercase text-sm">Contraseña</label>
									<input type="password" placeholder="" name="password" class="form-control mb">
